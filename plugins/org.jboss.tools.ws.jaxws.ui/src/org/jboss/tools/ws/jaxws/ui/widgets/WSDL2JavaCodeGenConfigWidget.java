 ...
 ******************************************************************************/ 
package org.jboss.tools.ws.jaxws.ui.widgets;
...
import org.jboss.tools.ws.jaxws.ui.commands.WSDL2JavaHelpOptionCommand;
import org.jboss.tools.ws.jaxws.ui.utils.JBossCreationUIUtils;
...
public class WSDL2JavaCodeGenConfigWidget extends SimpleWidgetDataContributor {
	...
	public WidgetDataEvents addControls(Composite parent,
		...
		serviceCombo = JBossCreationUIUtils.createComboItem(configCom, model,
				JBossJAXWSUIMessages.Label_Service_Name,
				JBossJAXWSUIMessages.Tooltip_Service);
		serviceCombo.addListener(SWT.Modify, new Listener() {
			public void handleEvent(Event arg0) {
				...
				if (service == null) {
					...
					status = StatusUtils.errorStatus(JBossJAXWSUIMessages.Error_Message_No_ServicePort);
		...
		sourceCombo = JBossCreationUIUtils.createComboItem(configCom, model,
				JBossJAXWSUIMessages.Label_SourceFolder_Name,
				JBossJAXWSUIMessages.Tooltip_SourceFolder);
		...
		customPacText = JBossCreationUIUtils.createTextItemWithButton(configCom, model,
				JBossJAXWSUIMessages.Label_Custom_Package_Name,
				JBossJAXWSUIMessages.Tooltip_Custom_Package);
		customPacButton = new Button(configCom, SWT.NONE);
		customPacButton.setText(JBossJAXWSUIMessages.Label__Browse_Button);
		...
		targetCombo = JBossCreationUIUtils.createComboItem(configCom, model,
				JBossJAXWSUIMessages.Label_JaxWS_Target,
				JBossJAXWSUIMessages.Tooltip_JaxWS_Target);
		...
		catalogText = JBossCreationUIUtils.createTextItemWithButton(configCom,
				model, JBossJAXWSUIMessages.Label_Catalog_File,
				JBossJAXWSUIMessages.Tooltip_Catalog_File);
		catalogButton = new Button(configCom, SWT.NONE);
		catalogButton.setText(JBossJAXWSUIMessages.Label_Add_Button);
		...
		extensionButton = JBossCreationUIUtils.createCheckButton(
						configCom, JBossJAXWSUIMessages.Label_EnableSOAP12_Binding_Extension);
		...
		if (model.getWsScenario() != WebServiceScenario.CLIENT) {
			...
			genDefaultButton = JBossCreationUIUtils.createCheckButton(configCom,
					JBossJAXWSUIMessages.Label_Generate_Impelemtation);
			...
			updateWebxmlButton = JBossCreationUIUtils.createCheckButton(
					configCom, JBossJAXWSUIMessages.Label_Update_Webxml);
		...
		additionalText = JBossCreationUIUtils.createTextItemWithButton(configCom, model,
				JBossJAXWSUIMessages.Label_AdditionalOption_Name,
				JBossJAXWSUIMessages.Tooltip_AdditionalOption);
		additionalButton = new Button(configCom, SWT.NONE);
		additionalButton.setText(JBossJAXWSUIMessages.Label_Help_Button);
		additionalButton.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				String message = getAdditionalOptions(model);
				MessageDialog.openInformation(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(),
						JBossJAXWSUIMessages.AdditionalOption_Dialog_Title, message);
			}
		});
		additionalText.addModifyListener(new ModifyListener() {
			public void modifyText(ModifyEvent e) {
				if (!"".equals(additionalText.getText())) { //$NON-NLS-1$
					...
					status = StatusUtils.errorStatus(JBossJAXWSUIMessages.Error_Message_No_ServletName);
					statusListener.handleEvent(null);
				}
			}
		});
	...
	private void createBindingFileItem(Composite configCom) {
		...
		label.setText(JBossJAXWSUIMessages.Label_Binding_File);
		label.setToolTipText(JBossJAXWSUIMessages.Tooltip_BindingFile);
		bindingList = new List(configCom, SWT.BORDER | SWT.SCROLL_LINE
				| SWT.V_SCROLL | SWT.H_SCROLL);
		GridData gd = new GridData(GridData.FILL_HORIZONTAL);
		bindingList.setToolTipText(JBossJAXWSUIMessages.Tooltip_BindingFile);
		...
		btnSelect.setText(JBossJAXWSUIMessages.Label_Add_Button);
		...
		removeButton.setEnabled(false);
		removeButton.setText(JBossJAXWSUIMessages.Label_Remove_Button);
	...
	private void updateComposite() {
		...
		if (!isOK) {
			status = StatusUtils.errorStatus(JBossJAXWSUIMessages.Error_Message_No_Service);
		}

		isOK = JBossCreationUIUtils.populateSourceFolderCombo(sourceCombo, model.getSrcList());
		if (!isOK) {
			status = StatusUtils.errorStatus(JBossJAXWSUIMessages.Error_Message_No_SourceFolder);
		}
	...
	public static String getAdditionalOptions(ServiceModel model) {
		IStatus status = null;
		String message = JBossJAXWSUIMessages.No_Message_AdditionalOptions_Dialog;
