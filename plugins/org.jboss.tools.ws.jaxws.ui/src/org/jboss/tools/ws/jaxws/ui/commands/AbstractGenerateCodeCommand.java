...
abstract class AbstractGenerateCodeCommand extends AbstractDataModelOperation {
	...
	public IStatus execute(IProgressMonitor monitor, IAdaptable info)
		...
		try {
			...
			monitor.subTask(JBossJAXWSCoreMessages.Progress_Message_Generating);
			IStatus status = Status.OK_STATUS;

			IProject project = model.getJavaProject().getProject();
			JBossJAXWSUIPlugin.getDefault().setGenerateTime(System.currentTimeMillis());
			try {
				...
				if (!path.toFile().getAbsoluteFile().exists()) {
					return StatusUtils.errorStatus(NLS.bind(JBossJAXWSCoreMessages.Error_Message_Command_File_Not_Found, new String[] { path.toOSString() }));
				}
				...
				if (exitValue != 0) {
					JBossJAXWSUIPlugin.getDefault().logError(errorResult.toString());
					JBossJAXWSUIPlugin.getDefault().logError(inputResult.toString());
					...
					if (resultInput != null) {
						...
						if (resultInput.indexOf("error:") >= 0 && !(resultInput.indexOf("compilation failed") >= 0)) { //$NON-NLS-1$ //$NON-NLS-2$
							JBossJAXWSUIPlugin.getDefault().logError(resultInput);
							IStatus errorStatus = StatusUtils.errorStatus(resultInput);
							status = StatusUtils.errorStatus(JBossJAXWSCoreMessages.Error_Message_Failed_To_Generate_Code,new CoreException(errorStatus));
							return status;
						}
						if (resultInput.indexOf("[ERROR]") >= 0) { //$NON-NLS-1$
							JBossJAXWSUIPlugin.getDefault().logWarning(resultInput);
							IStatus errorStatus = StatusUtils.warningStatus(resultInput);
							status = StatusUtils.warningStatus(JBossJAXWSCoreMessages.Error_Message_Failed_To_Generate_Code, new CoreException(errorStatus));
						}
