...
public class JBossWSRuntimeConfigBlock {
	...
	public Composite createControl(Composite parent) {
		...
		lblServerSupplied.setText(JBossJAXWSUIMessages.JBossWSFacetInstallPage_ServerSuppliedJBossWS);
		...
		btnNew.setText(JBossJAXWSUIMessages.JBossWSFacetInstallPage_New);
		...
		btnDeploy.setText(JBossJAXWSUIMessages.JBossWSFacetInstallPage_Deploy);
	...
	private void validateandDisplayTargetRuntime() {
		...
		if (runtime == null) {
			setErrorMessage(JBossJAXWSUIMessages.JBossWSRuntimeInstallPage_NoTargetRuntime);
			return;
		}
		...
		if (runtimeTypeId.indexOf(JBOSS_IDE_AS_RUNTIME_ID) < 0
			...
			setErrorMessage(JBossJAXWSUIMessages.JBossWSRuntimeInstallPage_NoValidJBossWSRuntime);
			return;
		}
	...
	protected void changePageStatus() {
		...
		if (btnUserSupplied.getSelection()
			...
			setErrorMessage(JBossJAXWSUIMessages.Error_WS_No_Runtime_Specifed);
			...
			if (!duplicateMsg.equals("")) { //$NON-NLS-1$
				setErrorMessage(MessageFormat.format(JBossJAXWSUIMessages.Error_JBossWSRuntimeConfigBlock_Duplicated_Jar,duplicateMsg));
			} else {
				setErrorMessage(null);
			}
			if (jr != null) {
	...
	protected String getDuplicateJars(String jbwsName)
		...
		for (IClasspathEntry entry : entries) {
			if (entry.getEntryKind() == IClasspathEntry.CPE_CONTAINER) {
				if (JBossJAXWSUIMessages.JBossWS_Runtime_Lib.equals(entry
						.getPath().segment(0))) {
					continue;
				}
