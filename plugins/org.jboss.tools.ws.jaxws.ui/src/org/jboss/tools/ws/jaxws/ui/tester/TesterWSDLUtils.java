 ...
 ******************************************************************************/
package org.jboss.tools.ws.jaxws.ui.tester;
...
public class TesterWSDLUtils {
	...
	public static IStatus isWSDLAccessible(URL contextURL) {
		...
		try {
			...
			if (contextURL.getProtocol().equalsIgnoreCase("https")) { //$NON-NLS-1$
				return StatusUtils.warningStatus(JBossJAXWSUIMessages.TesterWSDLUtils_WSDL_HTTPS_Secured_Inaccessible);
			} else {
				return StatusUtils.errorStatus(JBossJAXWSUIMessages.TesterWSDLUtils_WSDL_Inaccessible, e);
			}
		}
	...
	public static String[] getNSServiceNameAndMessageNameArray(Definition wsdlDefinition, String serviceName,
			String portName, String bindingName, String opName) {
		Map<?, ?> services = wsdlDefinition.getServices();
		Set<?> serviceKeys = services.keySet();
		for (Iterator<?> it = serviceKeys.iterator(); it.hasNext();) {
			QName serviceKey = (QName) it.next();
	...
	public static String getEndpointURL(Definition wsdlDefinition, String serviceName, String portName,
	...
	public static String getActionURL(Definition wsdlDefinition, String serviceName, String portName,
			String bindingName, String opName) {
		Map<?, ?> services = wsdlDefinition.getServices();
		Set<?> serviceKeys = services.keySet();
		for (Iterator<?> it = serviceKeys.iterator(); it.hasNext();) {
