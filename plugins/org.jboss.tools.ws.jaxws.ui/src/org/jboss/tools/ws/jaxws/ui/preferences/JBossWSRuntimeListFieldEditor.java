...
package org.jboss.tools.ws.jaxws.ui.preferences;
...
import org.jboss.tools.ws.jaxws.ui.utils.JBossWSUIUtils;
...
public class JBossWSRuntimeListFieldEditor extends BaseFieldEditor {
	...
	protected void createTableView() {
		tableView = CheckboxTableViewer.newCheckList(root, SWT.V_SCROLL | SWT.BORDER | SWT.FULL_SELECTION | SWT.SINGLE);
	...
	protected void createActionBar() {
		actionPanel = new ActionPanel(root, new BaseAction[] { new AddAction(), new EditAction(), new RemoveAction() });
	...
	private void setDefaultRuntime() {
		...
		if (!checked && runtimes.size() > 0) {
			if (tableView.getCheckedElements() == null || tableView.getCheckedElements().length == 0) {
	...
	public void doFillIntoGrid(Object parent) {
		Assert.isTrue(parent instanceof Composite, JBossJAXWSUIMessages.Error_JBossWS_Basic_Editor_Composite);
		Assert.isTrue(((Composite) parent).getLayout() instanceof GridLayout,
				JBossJAXWSUIMessages.Error_JBossWS_Basic_Editor_Support);
	 ...
	 * 
	 */
	public static class JBossWSRuntimeWizardPage extends WizardPage implements PropertyChangeListener {
		...
		IFieldEditor name = IFieldEditorFactory.INSTANCE.createTextEditor(SRT_NAME,
				JBossJAXWSUIMessages.JBossWS_Runtime_List_Field_Editor_Name2, ""); //$NON-NLS-1$

		IFieldEditor version = IFieldEditorFactory.INSTANCE.createComboEditor(SRT_VERSION,
				JBossJAXWSUIMessages.JBossWS_Runtime_List_Field_Editor_Version, getWSFacetVersions(), ""); //$NON-NLS-1$

		IFieldEditor homeDir = IFieldEditorFactory.INSTANCE.createBrowseFolderEditor(SRT_HOMEDIR,
				JBossJAXWSUIMessages.JBossWS_Runtime_List_Field_Editor_Home_Folder, ""); //$NON-NLS-1$
		...
		public JBossWSRuntimeWizardPage(List<JBossWSRuntime> editedList) {
			super(JBossJAXWSUIMessages.JBossWS_Runtime_List_Field_Editor_New_Runtime);
			setMessage(JBossJAXWSUIMessages.JBossWS_Runtime_List_Field_Editor_Create_A_Runtime);
			setTitle(JBossJAXWSUIMessages.JBossWS_Runtime_List_Field_Editor_Runtime);
			value = editedList;
		}
		...
		/**
		 * Process evt: setup default values based on JBossWS Home folder and validate
		 * user input
		 * 
		 * @param evt
		...
		public void propertyChange(java.beans.PropertyChangeEvent evt) {
			...
			if (name.getValueAsString() == null || "".equals(//$NON-NLS-1$
				...
				setErrorMessage(JBossJAXWSUIMessages.Error_JBossWS_Runtime_List_Field_Editor_Name_Cannot_Be_Empty);
				setPageComplete(false);
				return;
			}
			...
			if (!name.getValueAsString().matches("[a-zA-Z_][a-zA-Z0-9_\\-\\. ]*")) { //$NON-NLS-1$
				setErrorMessage(
						JBossJAXWSUIMessages.Error_JBossWS_Runtime_List_Field_Editor_Runtime_Name_Is_Not_Correct);
				setPageComplete(false);
				return;
			}
			for (JBossWSRuntime rt : value) {
				...
				if (rt.getName().equals(name.getValueAsString())) {
					setErrorMessage(
							NLS.bind(JBossJAXWSUIMessages.JBossWS_Runtime_List_Field_Editor_Runtime_Already_Exists,
			...
			if (jars != null) {
				...
				if (jarJbws.isUserConfigClasspath() && jarJbws.getLibraries().size() == 0) {
					setErrorMessage(JBossJAXWSUIMessages.JBossRuntimeListFieldEditor_ErrorMessageAtLeastOneJar);
					setPageComplete(false);
					return;
				}
			}
			...
			if (homeDir.getValueAsString() == null || "".equals(homeDir.getValueAsString().trim())) { //$NON-NLS-1$
				setErrorMessage(
						JBossJAXWSUIMessages.Error_JBossWS_Runtime_List_Field_Editor_Path_To_Home_Diretory_Cannot_Be_Empty);
				setPageComplete(false);
				return;
			}

			if (!runtimeExist(homeDir.getValueAsString())) {
				setErrorMessage(JBossJAXWSUIMessages.Error_JBossWS_Label_Runtime_Load);
				setPageComplete(false);
				return;
		...
		public ButtonFieldEditor.ButtonPressedAction createSelectFolderAction(String buttonName) {
			return new ButtonFieldEditor.ButtonPressedAction(buttonName) {
				...
				public void run() {
					...
					dialog.setMessage(JBossJAXWSUIMessages.JBossWS_SWT_Field_Editor_Factory_Select_Home_Folder);
					dialog.setFilterPath(getFieldEditor().getValueAsString());
		...
		private boolean runtimeExist(String path) {
			...
			if (!jbosswsHomeDir.isDirectory())
				return false;
			String[] newNode = { JBossJAXWSUIMessages.Bin, JBossJAXWSUIMessages.Command };
	...
	public static class JBossWSRuntimeNewWizard extends Wizard {
		...
		public JBossWSRuntimeNewWizard(List<JBossWSRuntime> exist, List<JBossWSRuntime> added) {
			super();
			setWindowTitle(JBossJAXWSUIMessages.JBossWS_Runtime_List_Field_Editor_New_Runtime);
	...
	/**
	 * Wizard for editing JBossWS Runtime parameters: name and path to home folder
	 * 
	 */
	public static class JBossWSRuntimeEditWizard extends Wizard {
		 ...
		 *            List&lt;JBossWSRuntime&gt; - TBD
		 */
		public JBossWSRuntimeEditWizard(List<JBossWSRuntime> existing, JBossWSRuntime source,
				List<JBossWSRuntime> added, Map<JBossWSRuntime, JBossWSRuntime> changed) {
			super();
			setWindowTitle(JBossJAXWSUIMessages.JBossWS_Runtime_List_Field_Editor_Edit_Runtime);
			page1 = new JBossWSRuntimeWizardPage(existing);
			page1.setMessage(JBossJAXWSUIMessages.JBossWS_Runtime_List_Field_Editor_Modify_Runtime);
			page1.setTitle(JBossJAXWSUIMessages.JBossWS_Runtime_List_Field_Editor_Edit_Runtime);
			addPage(page1);
			this.value = existing;
			this.added = added;
	 ...
	 * 
	 */
	public static class ActionPanel extends Composite implements ISelectionChangedListener {

		private BaseAction[] actions = null;

		/**
		 * Constructor creates panel with style, grid layout and buttons represented the
		 * actions
		 * 
		 * @param parent
		 *            Composite
		...
		/**
		 * Listen to the selection changes and update actions state (enable/disable)
		 * 
		 * @param event
	...
	public static class ActionButton implements IPropertyChangeListener {
		...
		/**
		 * Create Button control with parent control and style that represents action
		 * 
		 * @param parent
		 *            Composite
		 * @param style
		...
		public ActionButton(Composite parent, int style, BaseAction action) {
			...
			GridData gd = new GridData(GridData.FILL_HORIZONTAL, GridData.CENTER, false, false);
	...
	public class AddAction extends BaseAction {
		 ...
		 */
		public AddAction() {
			super(JBossJAXWSUIMessages.JBossRuntimeListFieldEditor_ActionAdd);
		...
		@Override
		public void run() {
			Wizard wiz = new JBossWSRuntimeNewWizard((List<JBossWSRuntime>) getValue(), added);
	...
	public class EditAction extends BaseAction {
		 ...
		 */
		public EditAction() {
			super(JBossJAXWSUIMessages.JBossRuntimeListFieldEditor_ActionEdit);
		}
		...
		/**
		 * Start editing selected JBossWS Runtime in Edit JBossWS Runtime Wizard Dialog
		 * 
		 * @see org.eclipse.jface.action.Action#run()
		 */
		@SuppressWarnings("unchecked")
		...
		public void run() {
			...
			Wizard wiz = new JBossWSRuntimeEditWizard((List<JBossWSRuntime>) getValue(), runtimes[0], added, changed);
			WizardDialog dialog = new WizardDialog(Display.getCurrent().getActiveShell(), wiz);
			dialog.open();
			tableView.refresh();
	...
	public class RemoveAction extends BaseAction {
		 ...
		 */
		public RemoveAction() {
			super(JBossJAXWSUIMessages.JBossRuntimeListFieldEditor_ActionRemove);
		}
		...
		private void removeRuntime(JBossWSRuntime r) {
			...
			String title = JBossJAXWSUIMessages.JBossWS_Runtime_Delete_Confirm_Title;
			String message = (used) ? NLS.bind(JBossJAXWSUIMessages.JBossWS_Runtime_Delete_Used_Confirm, r.getName())
					: NLS.bind(JBossJAXWSUIMessages.JBossWS_Runtime_Delete_Not_Used_Confirm, r.getName());
			boolean b = MessageDialog.openConfirm(tableView.getControl().getShell(), title, message);
			if (b) {
