...
import org.jboss.tools.ws.jaxws.core.data.ServletDescriptor;
...
public class JBossWSGenerateWizardValidator {
	...
	public static IStatus isWSNameValid() {
		...
		if (model.getWebProjectName() == null) {
			return StatusUtils
					.errorStatus(JBossJAXWSUIMessages.Error_JBossWS_GenerateWizard_NoProjectSelected);
		} else {
			try {
			...
			if (model != null) {
				for (int i = 0; i < descriptors.length; i++) {
					if (descriptors[i].getName().trim().length() == 0) {
						return StatusUtils
								.errorStatus(JBossJAXWSUIMessages.Error_JBossWS_GenerateWizard_ServiceName_Empty);
					}
					...
					for (int j = 0; j < theServlets.size(); j++) {
						...
						if (aServlet.getServletName().equals(
							...
							return StatusUtils
									.errorStatus(JBossJAXWSUIMessages.Error_JBossWS_GenerateWizard_WSName_Same);
						}
					}
	...
	public static IStatus isWSClassValid(String className, IProject project) {
		if (model.getCustomPackage().trim().length() == 0) {
			// empty package name
			return StatusUtils
					.errorStatus(JBossJAXWSUIMessages.Error_JBossWS_GenerateWizard_PackageName_Cannot_Be_Empty);
		} else if (model.getCustomClassName().trim().length() == 0) {
			// empty class name
			return StatusUtils
					.errorStatus(JBossJAXWSUIMessages.Error_JBossWS_GenerateWizard_ClassName_Cannot_Be_Empty);
			...
			if (file != null) {
				...
				return StatusUtils
						.errorStatus(JBossJAXWSUIMessages.Error_JBossWS_GenerateWizard_ClassName_Same);
			}
			return status;
		}
	}
	...
	private static ServletDescriptor getServletDescriptor(String clsName) {
		...
		sd.setMappings(JBossJAXWSUIMessages.Separator_Java
				+ sd.getDisplayName());
		return sd;
	}
