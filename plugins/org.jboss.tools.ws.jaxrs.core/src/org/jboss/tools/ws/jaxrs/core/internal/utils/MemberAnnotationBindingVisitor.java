...
public class MemberAnnotationBindingVisitor extends ASTVisitor {
	...
	/**
	 * Full Constructor to resolve annotation binding from its fully qualified name.
	 * 
	 * @param mName
	 *            the member name
	 * @param mType
	...
	private void visitExtendedModifiers(final List<?> modifiers) {
		for (Object modifier : modifiers) {
			if (modifier instanceof Annotation) {
				...
				if (annotationName != null && (annotationName.equals(binding.getAnnotationType().getQualifiedName())
						|| annotationName.equals(binding.getAnnotationType().getName()))) {
