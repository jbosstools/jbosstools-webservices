...
public class JBossRSGenerateWizardValidator {
			...
			if (model != null) {
				...
				for (int i = 0; i < theContextParams.size(); i++) {
					...
					if (pvalue.getParamName().equals(RSMergeWebXMLCommand.RS_APPLICATION_PARM_NAME)) {
						return new Status(IStatus.ERROR, JBossJaxrsUIPlugin.PLUGIN_ID, JBossWSUIMessages.JBossRSGenerateWizardValidator_ERROR_Can_Only_Add_Sample_Once);
					}
				}
			}
			return null;
				...
				for (int j = 0; j < theContextParams.size(); j++) {
					...
					if (pvalue.getParamName().equals(RSMergeWebXMLCommand.RS_APPLICATION_PARM_NAME)) {
						if (pvalue.getParamValue().equals(appClassName)) {
							new Status(IStatus.ERROR, JBossJaxrsUIPlugin.PLUGIN_ID, JBossWSCreationCoreMessages.Error_JBossWS_GenerateWizard_WSName_Same);
						}
					}
				}
				return null;
	...
	public static IStatus isWSNameValid() {
		IModelProvider provider = null;
		if (model.getWebProjectName() == null) {
			new Status(IStatus.ERROR, JBossJaxrsUIPlugin.PLUGIN_ID, JBossWSUIMessages.Error_JBossWS_GenerateWizard_NoProjectSelected);
		} else {
			try {
				IProject project = JBossWSCreationUtils.getProjectByName(model
						.getWebProjectName());
			...
			if (model != null) {
				for (int i = 0; i < descriptors.length; i++) {
					if (descriptors[i].getName().trim().length() == 0) {
						new Status(IStatus.ERROR, JBossJaxrsUIPlugin.PLUGIN_ID, JBossWSUIMessages.Error_JBossWS_GenerateWizard_ServiceName_Empty);
					}
