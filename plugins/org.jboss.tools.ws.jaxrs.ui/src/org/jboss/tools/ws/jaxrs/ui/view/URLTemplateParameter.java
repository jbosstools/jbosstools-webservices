...
public class URLTemplateParameter {
	...
	public String validate() {
		...
		// mandatory non-regex, but missing
			...
			final String errorMessage = JBossJAXRSUIMessages.WSTesterURLInputsDialog_Validation_Error_Missing_Value;
			return NLS.bind(errorMessage, new String[] { this.name });
		} else if (!this.value.matches(regex)) {
			final String errorMessage = JBossJAXRSUIMessages.WSTesterURLInputsDialog_Validation_Error_Invalid;
	...
	public String getReplacementContent() {
		...
		// or a question mark ('?'), depending on the type of parameter
			...
			case MATRIX_PARAM:
				return ';' + name + '=' + value;
			case QUERY_PARAM:
				return originalContent.charAt(0) + name + '=' + value;
			default:
				return value.trim();
			}
		}
